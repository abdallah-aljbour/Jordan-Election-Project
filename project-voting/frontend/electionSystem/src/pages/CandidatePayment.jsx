...
function CandidatePayment() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const debate = JSON.parse(sessionStorage.getItem("debate"));
      if (debate) {
        await axios.post(
          "http://localhost:4026/api/debates/createDebate",
          debate,
          { headers: { authorization: localStorage.getItem("token") } }
        );
        setSuccessMessage("تم انشائ مناظرة بنجاح!");
        setError(null);
      } else {
        await axios.put(
          `http://localhost:4026/api/updateAdvertisment/${advertismentID}`,
          {
            isPaid: true,
          }
        );
  ...
  return (
    <div className="p-4 mt-8 border border-gray-200 rounded-lg shadow-sm bg-gray-50">
      <h1 className="text-5xl font-bold text-center text-[#CE1126] mb-12">
        عملية الدفع
      </h1>
      ...
      <div className="p-8 mb-12 bg-white rounded-lg shadow-lg">
        <div className="flex items-center justify-center mb-6">
          <svg
            className="text-[#007A3D] w-12 h-12"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
        ...
        <h3 className="text-2xl font-semibold text-center text-[#007A3D] mb-4">
          دفع آمن للإعلانات
        </h3>
        <p className="text-center text-gray-600">
          نقدم عملية دفع آمنة لضمان حماية بياناتك أثناء دفع رسوم الإعلان. يتم
          استخدام التشفير من البداية إلى النهاية لضمان أمان المعلومات الشخصية
          وبيانات الدفع.
        </p>
      </div>
      <div className="my-6 sm:mt-8">
        <form
          ...
          className="w-full p-4 mx-auto bg-white border border-gray-200 rounded-lg shadow-sm lg:max-w-xl lg:p-8"
        >
